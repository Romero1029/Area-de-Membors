...
function openCalendar(aula: Aula) {
  ...
  if (isIOS) {
    const ics = ['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//IDM//SDW40//PT','BEGIN:VEVENT',
      `SUMMARY:${titulo}`,`DTSTART:${inicio}`,`DTEND:${fim}`,
      `DESCRIPTION:${descFull.replace(/\n/g,'\\n')}`,`URL:${aula.youtubeUrl}`,
      'END:VEVENT','END:VCALENDAR'].join('\r\n')
    const blob = new Blob([ics], { type: 'text/calendar;charset=utf-8' })
    const a = Object.assign(document.createElement('a'), { href: URL.createObjectURL(blob), download: `sdw40-aula${aula.id}.ics` })
    ...
    return (
      <div className="text-center space-y-4 py-2">
        ...
        <div>
          ...
          <p className="text-xs text-white/35 mt-0.5">Parabéns por concluir a Semana do Despertar #40.</p>
        </div>
...
// MAIN COMPONENT
// ─────────────────────────────────────────────
export function SemanaDespertar40({ firstName }: { firstName: string }) {
        ...
        <div className="w-full max-w-2xl mx-auto px-5 sm:px-8 pt-10 pb-24">
          ...
          <div className="mb-10">
            ...
            <div className="flex items-center justify-between border-t border-b border-white/[0.08] py-2.5 mb-7">
              ...
              <span className="text-[10px] uppercase tracking-[0.15em] text-white/25 border border-white/[0.12] px-2 py-0.5 rounded">SDW #40</span>
            </div>
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-white/75">Certificado da Semana do Despertar #40</p>
