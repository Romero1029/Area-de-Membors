...
import { ArrowLeft, Plus, Pencil, Trash2, Radio, ExternalLink, Eye, EyeOff } from 'lucide-react'
...
export function MentoriaNpaLivesClient({ initialLives }: { initialLives: Live[] }) {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Remover este encontro ao vivo?')) return
    setDeleting(id)
    await deleteLive(id)
  ...
  return (
    <div className="max-w-[820px] space-y-6">
      <Link href="/admin" className="inline-flex items-center gap-1.5 text-sm text-[#606060] hover:text-[#f0f0f0] transition-colors">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg bg-[#111111] rounded-2xl border border-[#2a2a2a] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e1e1e]">
          <h2 className="font-bold text-[#f0f0f0]">{live ? 'Editar encontro' : 'Novo encontro ao vivo'}</h2>
          <button onClick={onClose} className="text-[#606060] hover:text-[#f0f0f0] text-xl leading-none">×</button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          {field('Título *', 'title', { defaultValue: live?.title, required: true, placeholder: 'Ex: Encontro 01 — Abertura' })}
